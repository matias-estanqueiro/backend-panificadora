...
const addInsumo = async (req, res) => {
  try {
    ...
    const insumos = await readData('insumos')
    const existente = insumos.find(p => p.codigo === codigo);
    if (existente) {
      if (existente.activo !== false) {
        return res.status(409).json({ error: true, codigo_http: 409, mensaje: 'El insumo ya existe.' });
      } else {
        ...
        existente.punto_pedido = punto_pedido;
        await writeData('insumos', insumos);
        return res.status(200).json({ mensaje: 'El insumo ha sido reactivado y actualizado exitosamente.', insumo: existente });
      }
    }
    const id = uuidv4()
...
const updateInsumo = async (req, res) => {
  try {
    ...
    if (insumos[index].activo === false) {
      return res.status(409).json({ error: true, codigo_http: 409, mensaje: 'No se puede editar un insumo inactivo.' });
    }
    const validacion = insumoSchema.safeParse(req.body)
    if (!validacion.success) {
      return res.status(400).json({
